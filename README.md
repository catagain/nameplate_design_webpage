...
一個功能強大的網頁應用，用於建立、編輯和下載專業的會議桌上名牌（會議牌）。
...
**Q: 能否修改名牌尺寸？**
A: 當前固定為 1000×300px (20cm×6cm)。如需修改，編輯 `renderer.js` 中的 canvas 設置。

**Q: 支持哪些圖片格式？**
A: 支持所有主流格式：PNG、JPG、GIF、WebP、SVG 等。

**Q: 下載的 PNG 能列印嗎？**
A: 可以。1000×300px @150dpi 的 PNG 適合 20cm×6cm 的標準會議牌列印。

**Q: 如何集成到我的會議系統？**
A: 參考上面的 API 集成指南，或在 `app.js` 中的 `handleUpload()` 函數中自訂義接口。

**Q: 能否支持中文/日文/韓文？**
A: 已完全支持所有 Unicode 字符，包括中日韓文字。
