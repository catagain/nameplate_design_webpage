...
- 批量生產流程簡化
  - 原 3 步驟（匯入資料 → 檢視與編輯 → 批量輸出）合併為 2 步驟
  - 將「上傳 CSV」與「載入已儲存檔案」直接整合進資料編輯頁面
  - 步驟指示器同步更新為「匯入資料與編輯 → 批量輸出」
  - 「返回編輯頁」按鈕改為浮動固定在畫面左上角，無底色無底線
  - 隱藏欄位預覽區塊與狀態列，版面更簡潔
  - 「下載表格 CSV」按鈕在無資料時仍可點擊，下載僅含欄位名稱的空白 CSV
...
- 抖動運算調整
  - 預設改為關閉
  - 當使用者選用非預設顏色或上傳圖片時，彈出建議開啟抖動運算的對話窗
  - 對話窗內含勾選項，勾選後即時啟用抖動運算

- 預設顏色選項調整
  - 色票由 10 色縮減為 6 色：純黑、正紅、正藍、正綠、正黃、純白
  - 6 色皆為高飽和度設定（s: 95）
  - 每色自帶深色版本（第二排），取代原有的淡色版本
  - 去除純白、純黑以外的低飽和色票，減少抖動畫面的白點
  - 深色版本亮度統一調整至 l=30，各色深色版視覺亮度一致

- 快速預設調整
  - 預設改為四組：白底黑字、深藍色底白字、黑底白字、白底深藍字
  - 所有顏色皆限定使用預設六色與其深色版本

- 模擬桌牌顯示
  - 預覽畫面右下角新增「模擬桌牌」勾選項
  - 啟用時 canvas 亮度降低至 82%，模擬桌牌實際顯示效果
  - 預設開啟，狀態可持久化儲存
